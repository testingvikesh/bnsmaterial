...
class MaterialWebsiteDraft
{
    ...
    public static function for(array $facts): array
    {
        ...
        $services = self::val($facts, 'services');
        ...
        $offer = self::shortLine($product);
        $context = compact('biz', 'cat', 'product', 'intro', 'services', 'city', 'address', 'phone', 'member', 'family', 'cta', 'offer') + ['facts' => $facts];
        ...
        return [
            ...
            'service_points' => MaterialCopyPoints::from($services),
    ...
    {
        $facts = $context['facts'];
        $introPoints = MaterialCopyPoints::from($context['intro']);
        $productPoints = MaterialCopyPoints::from($context['product']);
        $servicePoints = MaterialCopyPoints::from($context['services']);

        $profile = [
            'member_name' => self::field($context['member'] ?: 'Member name required', $context['member'] !== '' ? self::VERIFIED : self::REQUIRED),
            'member_id' => self::field(self::val($facts, 'member_id') ?: 'Member ID required', self::val($facts, 'member_id') ? self::VERIFIED : self::REQUIRED),
            'phone' => self::field($context['phone'] ?: 'Phone required', $context['phone'] !== '' ? self::VERIFIED : self::REQUIRED),
            ...
            'address' => self::field($context['address'] ?: 'Address required', $context['address'] !== '' ? self::VERIFIED : self::REQUIRED),
            'website' => self::field(self::val($facts, 'website') ?: 'Website URL required', self::val($facts, 'website') ? self::VERIFIED : self::REQUIRED),
            'instagram' => self::field(self::val($facts, 'instagram') ?: 'Instagram not provided', self::val($facts, 'instagram') ? self::VERIFIED : self::REQUIRED),
            'facebook' => self::field(self::val($facts, 'facebook') ?: 'Facebook not provided', self::val($facts, 'facebook') ? self::VERIFIED : self::REQUIRED),
            'business_description' => self::field($context['intro'] ?: $context['biz'].' offers '.$context['offer'].' for customers who want a trusted '.$context['cat'].' partner.', $context['intro'] !== '' ? self::VERIFIED : self::INFERRED, $introPoints),
            'main_product' => self::field($context['product'] ?: 'Main product required', self::has($context['product']) ? self::VERIFIED : self::REQUIRED, $productPoints),
            'main_services' => self::field($context['services'] !== '' ? $context['services'] : 'Included with the main product / service list above.', $context['services'] !== '' ? self::VERIFIED : self::INFERRED, $servicePoints),
            ...
            'business_positioning' => self::field($context['biz'].' as a customer-first '.$context['cat'].' brand for '.$context['offer'].'.', self::INFERRED),
            'customer_need' => self::field(self::needLine($context), self::INFERRED),
            'market_context' => self::field(($context['city'] !== '' ? $context['city'] : 'Local').' '.$context['cat'].' customers looking for reliable '.$context['offer'].'.', self::INFERRED),
        ];
    ...
    private static function section(string $code, array $context): array
    {
        ...
        $blocks = match ($code) {
            ...
            'signature' => [
                self::block('Flagship offering', $context['offer'], self::has($context['product']) ? self::VERIFIED : self::REQUIRED),
                self::block('How to present it', $context['biz'].' presents '.$context['offer'].' as the signature reason customers choose this brand.', self::INFERRED),
            ],
            'pricing' => [
            ...
            'quality' => [
                self::block('Quality promise', $context['biz'].' keeps '.$context['offer'].' simple, reliable and customer-friendly.', self::INFERRED),
            ...
            'membership' => [
                self::block('Loyalty idea', 'A simple repeat-customer club for '.$context['offer'].' can be added if it fits the business model.', self::SUGGESTED),
            ],
            'experience' => [
                self::block('Enquiry', 'Customer contacts '.$context['biz'].' by phone, WhatsApp or form.', self::INFERRED),
                self::block('Purchase / booking', 'Confirm requirement, share options, close the '.$context['offer'].' order.', self::INFERRED),
    ...
    private static function hero(array $context): array
    {
        ...
        return [
            ...
            self::block('Subheadline', $context['cat'].($place !== '' ? $place : ''), self::INFERRED),
            ...
            self::block('Hero image concept', 'A clean photograph of '.$context['offer'].' with the '.$context['biz'].' name and one clear '.$context['cta'].' button.', self::SUGGESTED),
        ];
    }
    ...
    private static function introduction(array $context): array
    {
        ...
        $productPoints = MaterialCopyPoints::from($context['product']);
        $overview = $points !== [] ? implode(' ', array_slice($points, 0, 2)) : $context['biz'].' is a '.$context['cat'].' business offering '.$context['offer'].'.';

        return [
            self::block('Who we are', $context['biz'].($context['city'] !== '' ? ', '.$context['city'] : '').' — '.$context['cat'], $context['intro'] !== '' ? self::VERIFIED : self::INFERRED),
            self::block('What we do', $productPoints !== [] ? implode("\n", $productPoints) : $context['offer'], self::has($context['product']) ? self::VERIFIED : self::REQUIRED, $productPoints),
            self::block('Business overview', $overview, $context['intro'] !== '' ? self::VERIFIED : self::INFERRED, $points),
            self::block('Key highlights', $context['offer'].' • Customer-first process • Clear next step to '.$context['cta'], self::INFERRED),
        ];
    }
    ...
    {
        return [
            self::block('Purpose', 'Help customers choose '.$context['offer'].' with clarity and trust.', self::INFERRED),
            self::block('Vision', 'Become the remembered '.$context['cat'].' name'.($context['city'] !== '' ? ' in '.$context['city'] : '').'.', self::INFERRED),
            self::block('Mission', 'Deliver '.$context['offer'].' that is simple to understand, easy to buy and reliable after purchase.', self::INFERRED),
            self::block('Values', 'Clarity, craftsmanship of service, honesty of claims, and long-term customer care.', self::INFERRED),
            self::block('Future direction', 'Strengthen digital enquiry, repeat customers and one signature '.$context['offer'].' offer.', self::SUGGESTED),
        ];
    }
    ...
    private static function problem(array $context): array
    {
        ...
        return [
            ...
            self::block('Expectations', 'Customers expect '.$context['biz'].' to explain '.$context['offer'].' simply and respond quickly.', self::INFERRED),
        ];
    }
    ...
    {
        return [
            self::block('Solution', $context['biz'].' uses '.$context['offer'].' to remove guesswork: understand the need, show a clear option, and complete the enquiry with a human conversation.', self::INFERRED),
        ];
    }
    ...
    private static function products(array $context): array
    {
        ...
        return [
            ...
            self::block('Main products', implode("\n", $list), self::has($context['product']) ? self::VERIFIED : self::REQUIRED, $list),
            self::block('Benefits', 'Customers get a clear '.$context['offer'].' option, guided selection and a simple next step.', self::INFERRED),
    ...
    private static function services(array $context): array
    {
        ...
        return [
            self::block('Services', $points !== [] ? implode("\n", $points) : ($context['services'] ?: $context['offer']), self::has($context['product']) ? self::VERIFIED : self::INFERRED, $points),
    ...
    {
        return [
            self::block('Product innovation', 'Potential Navachar Opportunity: a named signature '.$context['offer'].' collection or package.', self::SUGGESTED),
    ...
    {
        return [
            self::block('What do you offer?', $context['biz'].' offers '.$context['offer'].'.', self::INFERRED),
    ...
    private static function seo(array $context): array
    {
        ...
        return [
            'primary_keyword' => $offer.$place,
            'secondary_keywords' => $context['cat'].', '.$offer.', '.$context['biz'],
            'local_keywords' => $context['cat'].$place,
            'seo_title' => $context['biz'].' | '.$offer.$place,
            'meta_description' => $context['biz'].' offers '.$offer.($place !== '' ? ' in'.$place : '').'. Enquire for a clear next step.',
            'h1' => $context['biz'],
            'h2' => $offer.' for customers who want a trusted '.$context['cat'].' partner',
            'faq_keywords' => $offer.' price, '.$offer.' process, '.$context['biz'].' contact',
            'image_alt' => $context['biz'].' '.$offer.' '.$context['cat'],
            'url_slug' => $slug,
        ];
    }
     ...
     * @return array<string, mixed>
     */
    private static function block(string $label, string $text, string $status, array $points = []): array
    {
        if ($points === [] && (str_contains($text, "\n") || mb_strlen($text) > 160)) {
     ...
     * @return array<string, mixed>
     */
    private static function field(string $text, string $status, array $points = []): array
    {
        return self::block('', $text, $status, $points);
    }
    ...
    private static function taglineFor(array $context): string
    {
        return match ($context['family']) {
            ...
            default => $context['offer'].' you can trust',
        };
    }
    ...
    private static function uspFor(array $context): string
    {
        if ($context['intro'] !== '') {
            ...
            if (($points[0] ?? '') !== '' && mb_strlen($points[0]) <= 160) {
                return $points[0];
            }
        }

        return $context['biz'].' makes '.$context['offer'].' easy to understand and easy to start.';
    }
    ...
    private static function targetCustomer(array $context): string
    {
        return match ($context['family']) {
            ...
            default => 'Customers who need '.$context['offer'].' with a trusted local partner',
        };
    }
    ...
    {
        return 'A clear, trustworthy way to choose '.$context['offer'].' without confusion.';
    }
    ...
    private static function useCases(array $context): string
    {
        return match ($context['family']) {
            ...
            default => 'First-time buyers, repeat customers and people comparing '.$context['offer'].' options',
        };
    }
    ...
    {
        return $kind === 'service'
            ? 'Consultation, customisation, after-care and repeat-order support around '.$context['offer']
            : 'Related '.$context['cat'].' options that sit next to '.$context['offer'];
    }
